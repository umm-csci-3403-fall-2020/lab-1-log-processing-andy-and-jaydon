...
find . -name "failed_login_data.txt" | xargs grep -rIhEo '\b[a-z][-a-zA-Z0-9_]*' | sort | uniq -c >> TEMP.txt #Grabs text from all files in directory that match the pattern, then sorts it and puts the names into their own lines, then counts the times each name is found and puts it in a file
