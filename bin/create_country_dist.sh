...
awk 'NF {print "data.addRow([\047" $2"\047, " $1"]);"}' TMP2 | tee TMP3) #Takes the earlier output and creates a new line using the variables defined in awk and puts the output into a file.
./bin/wrap_contents.sh "$DIR"/TMP3 html_components/"$FILE" "$FILE".html #Wrap temp file contents in html and create html file
